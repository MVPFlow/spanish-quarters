...
export const MemoryHallway = ({
  ...
  const externalMedia = !isSpecialMemory
  ...
  const mediaAssets = isSpecialMemory ? SPANISH_MEMORY_ASSETS : [];

  const hallwayLength = isSpecialMemory
    ? Math.max(
    ...
    : hasExternalMedia
      ? 72
      : 50;
  const wallHeight = isSpecialMemory ? 12 : hasExternalMedia ? 11 : 10;
  const ceilingHeight = wallHeight;

  const ceilingLightPositions = useMemo(() => {
    if (!isSpecialMemory) return [-18, -8, 2, 12];
  ...
  }, [hallwayLength, isSpecialMemory, mediaAssets.length]);

  const featuredMedia = useMemo<HallwayMedia[]>(() => {
    if (!isSpecialMemory) return SAMPLE_MEMORIES.map((memory) => ({ ...memory }));
  ...
  }, [hallwayLength, isSpecialMemory, mediaAssets]);
  ...
  useEffect(() => {
    ...
    if (videoPlayingCount > 0 && isSpecialMemory) {
      audio.pause();
    } else if (!muted && unlocked && isSpecialMemory) {
      audio.play();
    }
  }, [isSpecialMemory, muted, unlocked, videoPlayingCount]);

  return (
    <group>
      <ambientLight intensity={isSpecialMemory ? 0.75 : 0.5} />
      <hemisphereLight
        color="#f4d8b8"
        groundColor="#14110f"
        intensity={isSpecialMemory ? 0.9 : 0.65}
      />
      ...
      {ceilingLightPositions.map((z, i) => (
        ...
        >
          <pointLight
            ...
            distance={isSpecialMemory ? 18 : 14}
            decay={2}
            color={isSpecialMemory ? "#ffd8ac" : "#ffd2a0"}
          />
          <mesh>
            <sphereGeometry args={[0.11, 16, 16]} />
            <meshBasicMaterial color={isSpecialMemory ? "#ffd8ac" : "#ffd2a0"} />
          </mesh>
        </group>
      ))}
        ...
        <>
          <group position={[0, 2, -4]}>
            <PositionalAudio
              ...
              url="/audio/leberch-documentary-cinematic-241576.mp3"
              distance={80}
              loop
      ...
      {isSpecialMemory &&
        featuredMedia.map((media) =>
          media.asset?.type === "video" ? (
            ...
            <MemoryFrame key={media.id} data={media} />
          ),
        )}
      ...
      {!isSpecialMemory && !hasExternalMedia &&
        featuredMedia.map((media) => (
          <MediaFrame
            key={media.id}
            media={media}
...
const VideoFrame = ({
  ...
  return (
    <group position={media.position} rotation={media.rotation}>
      ...
      {!isLoaded && (
        <LoadingOverlay width={contentBounds.width} height={contentBounds.height} />
      )}
...
const ExternalVideoFrame = ({
  ...
  return (
    <group position={slot.position} rotation={slot.rotation}>
      ...
      {!isLoaded && <LoadingOverlay width={4.05} height={2.55} />}
      <Html center distanceFactor={13} transform>
        <iframe
          title={slot.title}
...
const LoadingOverlay = ({
  ...
  const spinnerScale = Math.min(width, height) * 0.16;
  return (
    <group position={[0, 0, 0.09]}>
      ...
      <mesh rotation={[0, 0, -Math.PI / 2]}>
        <torusGeometry args={[spinnerScale, spinnerScale * 0.18, 10, 32]} />
        <meshStandardMaterial
          color="#6bb0ff"
          emissive="#6bb0ff"
          emissiveIntensity={2}
